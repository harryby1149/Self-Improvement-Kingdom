...
module.exports = function (app, Op) {
  ...
  app.get("/api/friends", function (req, res) {
    db.sequelize.query("SELECT * FROM Users JOIN Friends ON username = requester OR username = requestee WHERE username = requester AND ? = requestee AND status = 'pending' OR '?' = requester AND username = requestee AND status = 'accepted';", { replacements: [req.session.username, req.session.username, req.session.username] })
